...
const updateString = async () => {
  ...
  const { adjectives } = await fetchAsync(
    "http://localhost:3000/data/adjectives"
  );

  const randomAdjective = adjectives[randomNumber(adjectives)];
  const seinVerb = Math.random() > 0.5 ? "sind" : "ist";
  const randomNoun = nouns[randomNumber(nouns)].singular;

  const generatedString = `Das ${seinVerb} ... (${randomNoun}). | ... (${randomNoun}) ${seinVerb} ${randomAdjective}.`;
